...
| **No schema registry.** A `version` field and discipline. `libs/pyevents` reimplements the TypeScript contract by hand, with nothing detecting drift. | Event sourcing makes drift permanent: malformed events sit in the log forever and replay on every rebuild. This is the largest architectural gap and it grows with every event written. | JSON Schema or Avro as the source of truth, generated for both languages, with a compatibility check in CI. |
